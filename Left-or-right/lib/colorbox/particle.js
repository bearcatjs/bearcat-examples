...
var moveRelative = require("model").moveRelative;
var geo = require("geometry");
var util = require("util");
var Klass = require("base").Klass;
var transformableTrait = require("transformable").transformableTrait;

var Space = Klass.extend([transformableTrait]);

var time = 0;
var ts = Klass.extend(undefined, {
...
var initAgent = function ()
{
  ...
  // Matrix等待被替换!!!!!
  this.slot("space", Space.create(ts));
  this.slot("_parentMatrix", geo.identityMatrix());

  var cm = CircleModel.create({
    radius:5, 
    fill: "rgba(255, 0, 0, 255)", 
    //ratioAnchorPoint: {x: 0.5, y:0.5}
  });
  cm = moveRelative(-0.5, -0.5, cm);
...
var TAgent = Trait.extend({
  ...
  {
    var inst = this.clone(traits, xMethods);
    //debug
    inst._name = this._name + " clone";

    // Matrix等待被替换!!!!!

    inst.slot("space", Space.create(ts));
  ...
  reset: function ()
  {
    ...
    space.exec("translate", 0, 0, 0);
    space.exec("scale", 1, 1);
    space.exec("rotate", 0);
  ...
  worldMatrix: function ()
  {
    ...
    var mat = this.slot("space").exec("matrix");
    mat = geo.matrixMult(this.slot("_parentMatrix"), mat);
    return mat;
  },
...
var Particle = Entity.clone([TAgent],{},initAgent);
...
var Emitter = Particle.clone([],{
  ...
  _processNewBorns: function (env, fromWhere)
  {
    ...
    var parentSpace = this.slot("space");
